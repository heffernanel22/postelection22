...
youth_election_map <- left_join(wv_counties,comparing_youth_voter_turnout_2018_2022) %>% filter(`decrease in youth voter turnout_adjusted`>0)


#Making an interactive map
#Creating palettes
pal1 <-colorQuantile(c("skyblue","#1A3C6C"),domain = youth_election_map$`decrease in youth voter turnout_adjusted`,n=5,na.color = "white")
numericPal <- colorNumeric(c("skyblue","#1A3C6C"),youth_election_map$`decrease in youth voter turnout_adjusted`)
                          
#Creating a hover effect
labels <- sprintf("<strong>%s:</strong><br/>%s percent decrease",as.character(youth_election_map$County),as.character(youth_election_map$`decrease in youth voter turnout_adjusted`),
as.character(youth_election_map$`decrease in youth voter turnout_adjusted`)) %>% lapply(htmltools::HTML)

#Creating the map 
wv_interactive_youth_election_map <- leaflet() %>% addProviderTiles("CartoDB.PositronNoLabels") %>% setView(-79.20,38.90,zoom =5.5) %>% addPolygons(data = youth_election_map,fillColor =~pal1(youth_election_map$`decrease in youth voter turnout_adjusted`),fillOpacity = 10,color = "white",weight = .6,highlightOptions = highlightOptions(
    weight = 3,
    color = "#FFFFFF",
    fillOpacity = 0.7,
  ...
  labelOptions = labelOptions(
    ...
    textsize = "15px")) %>% addLegendNumeric(pal = numericPal, values = youth_election_map$`decrease in youth voter turnout_adjusted`, title = 'Percent decrease in turnout',
...
wv_interactive_youth_election_map
